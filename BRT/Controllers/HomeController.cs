...
namespace BRT.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult UpdateBRTLocations()
        {
            if (TempData["Data"] != null)
            {
                // Deserialize the DataTable from the JSON stored in TempData
                string jsonData = TempData["Data"].ToString();
        ...
        public ActionResult UpdateBRTLocations2(DateTime _date)
        {
            if (_date != null)
            {
                DataTable dt = new DataTable();
                using (SqlConnection con = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    using (SqlCommand cmd = new SqlCommand("usp_updateBRTLocationsOnGatePass", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                ...
                // Serialize the DataTable into JSON and store it in TempData
                string jsonData = JsonConvert.SerializeObject(dt);
                TempData["Data"] = jsonData;
            }

            return RedirectToAction("UpdateBRTLocations", "Home");
        }

        [HttpPost]
        public ActionResult UpdateLocations(string SerializedTableData)
        {
            if (!string.IsNullOrEmpty(SerializedTableData))
            {
                // Deserialize the JSON back to a DataTable
                DataTable dt = JsonConvert.DeserializeObject<DataTable>(SerializedTableData);

                // Loop through the DataTable and update each record as needed
                using (SqlConnection con = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    con.Open();
                    foreach (DataRow row in dt.Rows)
                    {
                        using (SqlCommand cmd = new SqlCommand("usp_UpdateBrtLocationByContainerNo", con))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.AddWithValue("@ContainerNo", row["ContainerNo"]);
                            cmd.Parameters.AddWithValue("@LocationId", row["locationId"]);
                            cmd.Parameters.AddWithValue("@GatePassId", row["GatePassId"]);
                            cmd.Parameters.AddWithValue("@AddedBy", HttpContext.Session.GetString("Userid"));
                ...
                TempData["Message"] = "Locations updated successfully!";
            }

            return RedirectToAction("UpdateBRTLocations");
        }
